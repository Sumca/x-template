...
import { extractButtons } from '@/utils/btnPermission'
...
// 这里假设你已经有一个函数用来从后端获取路由数据
 const fetchRoutesFromBackend = async(): Promise<RouteRecordRaw[]> =>{
  ...
  const buttonsPersimmon = extractButtons(data)
  // 进行权限匹配
...
const hasPermission = (router: RouteRecordRaw, permissions: Permission[]): boolean => {
    // 查找权限
    const matchedPermission = permissions.find(item => item.name === router.name);
    // 如果找到了权限，检查子路由的权限
